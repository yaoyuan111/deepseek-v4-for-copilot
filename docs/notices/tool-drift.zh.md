...
当启用实验性设置 `personal-ai.experimental.stabilizeToolList` 时，扩展会尝试预先激活 VS Code/Copilot 的 `activate_*` 虚拟工具，让传给 DeepSeek API 的 `tools` 参数在多轮对话中更完整、更稳定。
...
2. 关闭 `personal-ai.experimental.stabilizeToolList`。
