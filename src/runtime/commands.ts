...
export function registerCommands(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		vscode.commands.registerCommand('personal-ai.showLogs', () => logger.show()),
		vscode.commands.registerCommand('personal-ai.openRequestDumpsFolder', () =>
			openRequestDumpsFolder(context),
		),
		vscode.commands.registerCommand('personal-ai.getApiKey', () =>
			vscode.env.openExternal(vscode.Uri.parse(EXTERNAL_URLS.deepseek.apiKeys)),
		),
		vscode.commands.registerCommand('personal-ai.openSettings', () =>
			vscode.commands.executeCommand('workbench.action.openSettings', 'personal-ai'),
		),
	);
}
